...
def again():    #for login window-----------------------------------------------------------------------------LOGIN WINDOW
    ...
    if flag=='apt':
        apt.destroy()
    root=Tk()
    root.title('PICT SOFTWARE SOLUTIONS')
    Label(root,text='SANCHETI CHEMIST AND DRUG SHOP',font ="Verdana 20 ").grid(row=0,column=0,columnspan=5)
    Label(root,text="PUNE",font ="Verdana 15").grid(row=1,column=0,columnspan=5)
    Label(root,text='-------------------------------------------------------').grid(row=2,column=0,columnspan=5)
    Label(root, text='Username',font ="Verdana 13").grid(row=3, column=0)
    un=Entry(root,width=20)
    un.grid(row=3, column=1)
    Label(root, text='Password',font ="Verdana 13").grid(row=4, column=0)
    pwd=Entry(root,width=20)
    pwd.grid(row=4, column=1)
    Button(root,width=8,text='Enter',command=check).grid(row=5, column=0)
    Button(root,width=8,text='Close',command=root.destroy).grid(row=5, column=1)
    root.mainloop()
...
def open_win(): #OPENS MAIN MENU
    ...
    apt=Tk()
    apt.title("Interface")
    Label(apt, text="SANCHETI CHEMIST AND DRUGSHOP").grid(row=0,column=0,columnspan=3)
...
def open_cus(): #OPENS MAIN MENU----------------------------------------------------------------------------MAIN MENU
    ...
    apt=Tk()
    apt.title("Interface")
    Label(apt, text="Sancheti CHEMIST AND DRUGGIST").grid(row=0,column=0)
...
def make_bill(): # makes bill
    ...
    m+="===============================================\n"
    m+="                                  No :%s\n\n" % det[5]
    m+="          Sancheti CHEMIST AND DRUGGIST\n"
    m+="  Pune Institute of Computer Technology\n\n"
    m+="-----------------------------------------------\n"
    if t==1:
        m+="Name: %s\n" % named
        m+="Address: %s\n" % addd
